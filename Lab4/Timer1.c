...
void Timer1_Init(void(*task)(void)){
  ...
  TIMER1_TAMR_R = 5000000;   // 3) configure for periodic mode, down-count 
...
void Timer0_Init(void(*task)(void)){
  ...
  TIMER0_TAMR_R = 50000000;   // 3) configure for periodic mode, down-count 
  TIMER0_TAILR_R = 0xFFFFFFFF;  // 4) reload value
  TIMER0_TAPR_R = 0;            // 5) bus clock resolution
  TIMER0_ICR_R = 0x00000001;    // 6) clear TIMER1A timeout flag
  TIMER0_IMR_R = 0x00000001;    // 7) arm timeout interrupt
  NVIC_PRI5_R = (NVIC_PRI5_R&0xFFFF00FF)|0x20000000; // 8) priority 4
// interrupts enabled in the main program after all devices initialized
// vector number 37, interrupt number 21
  NVIC_EN0_R = 1<<19;           // 9) enable IRQ 21 in NVIC
